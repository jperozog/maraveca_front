...
@Component({
  selector: 'app-detalles-mudanza',
  templateUrl: './detalles-mudanza.component.html',
  styleUrls: ['./detalles-mudanza.component.css']
})
export class DetallesMudanzaComponent implements OnInit {
  ...
  traerDatos() {
    this.instalacionesService.traerDatosMudanza(this.id).subscribe(res => { console.log(res), this.mudanza = res, this.historial = res["historial"] }, err => console.log(err))
  }

  CerrarMudanza() {
   
      this.cierreI = true
      this.celda = this.mudanza["lugar_muda"];
  ...
  cerrarMudanzaF() {
    ...
    this.instalacionesService.cerrarMudanzaF(this.id,
