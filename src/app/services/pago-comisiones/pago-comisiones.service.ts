...
export class PagoComisionesService {
  ...
  guardarData(comision:number,tasacambio:number,tipoComision: string,emisor:string, receptor:string, tipo:string, id_user:number,responsable:number,referencia:string,mes:number,anio:number,fechaPago:string){
    return this.http.post(environment.apiEndpoint+"guardarComision/",{comision,tasacambio,tipoComision,emisor,receptor,tipo,id_user,responsable,referencia,mes,anio,fechaPago})
  }
